...
#ifndef CUDA_INTERVAL_LIB_HPP_
#define CUDA_INTERVAL_LIB_HPP_

#include "cuda_interval_rounded_arith.hpp"
...
// don't consider rounding error for now
template<class T> inline __device__
interval_gpu<T> sqrt(interval_gpu<T> const &x)
{
    rounded_arith<T> rnd;
    return interval_gpu<T>(rnd.sqrt_down(x.upper()), rnd.sqrt_up(x.lower()));
}

// don't consider rounding error for now
template<class T> inline __device__
interval_gpu<T> exp(interval_gpu<T> const &x)
{
    return interval_gpu<T>(exp(x.upper()), exp(x.lower()));
}
...
template<class T> inline __device__
interval_gpu<T> operator+(interval_gpu<T> const &x, T const &y)
{
    rounded_arith<T> rnd;
    return interval_gpu<T>(rnd.add_down(x.lower(), y),
                           rnd.add_up(x.upper(), y));
}
...
{
    rounded_arith<T> rnd;
    return interval_gpu<T>(rnd.sub_down(x.lower(), y),
                           rnd.sub_up(x.upper(), y));
}

template<class T> inline __device__
interval_gpu<T> operator+(T const &x, interval_gpu<T> const &y)
{
    rounded_arith<T> rnd;
    return interval_gpu<T>(rnd.add_down(x, y.lower()),
                           rnd.add_up(x, y.upper()));
}
...
{
    rounded_arith<T> rnd;
    return interval_gpu<T>(rnd.sub_down(x, y.upper()),
                           rnd.sub_up(x, y.lower()));
}
...
interval_gpu<T> operator*(interval_gpu<T> const &x, T const &y)
{
    // Textbook implementation: 14 flops, but no branch.
    rounded_arith<T> rnd;
    return interval_gpu<T>(fmin(rnd.mul_down(x.lower(), y),
                                rnd.mul_down(x.upper(), y)),
                           fmax(rnd.mul_up(x.lower(), y),
                                rnd.mul_up(x.upper(), y)));
}
...
interval_gpu<T> operator*(T const &x, interval_gpu<T> const &y)
{
    // Textbook implementation: 14 flops, but no branch.
    rounded_arith<T> rnd;
    return interval_gpu<T>(fmin(rnd.mul_down(x, y.lower()),
                                rnd.mul_down(x, y.upper())),
                           fmax(rnd.mul_up(x, y.lower()),
                                rnd.mul_up(x, y.upper())));
}
...
interval_gpu<T> operator/(interval_gpu<T> const &x, T const &y)
{
    // Textbook implementation: 14 flops, but no branch.
    rounded_arith<T> rnd;
    return interval_gpu<T>(fmin(rnd.div_down(x.lower(), y),
                                rnd.div_down(x.upper(), y)),
                           fmax(rnd.div_up(x.lower(), y),
                                rnd.div_up(x.upper(), y)));
}
...
interval_gpu<T> operator/(T const &x, interval_gpu<T> const &y)
{
    ...
    return I(rnd.div_down(x, yl), rnd.div_up(x, yu));
}
